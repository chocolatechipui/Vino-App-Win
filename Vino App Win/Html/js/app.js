$(function () {
  ...
  var aboutContent = $('#aboutContent').html();
    ...
    var searchType = $('#searchTypeTemplate').html();
    var searchBody = $('#searchBody').html();
    searchBody
  ...
  $('.horizontal-scroll-panel').on('singletap', 'li', function() {
    ...
    $.publish('top-picks-selected-wine', tappedWine[0]);
  }); 

  // Define Mediator to output chosen wine:
  //=======================================
  var TopPicksMediator = $.subscribe('top-picks-selected-wine', function(event, wine) {
  ...
  var assembleSearchSheet = function () {
    $('#searchSheet section').html(searchNavBar + searchType + searchBody + priceSelector);
  ...
  $('#filteredWines').on('singletap', 'li', function() {
    ...
    $.publish('top-picks-selected-wine', selection[0]);
    $.UIGoToArticle('#selectedWine');
  });
  ...
  //========================================
  var WineryLocationMediator = $.subscribe('top-picks-selected-wine', function (event, winery) {
  ...
  var renderWineryMap = function(location) {
      var url = window.location.href = 'maps:' + location + ',CA,USA';
